...
export default function AdminPortal() {
  ...
  if (!authenticated) {
    return (
      <div className="flex min-h-screen items-center justify-center" style={{ background: 'hsl(30 20% 98%)' }}>
        <div className="w-full max-w-sm rounded-lg border bg-white px-7 py-8" style={{ borderColor: 'hsl(30 8% 90%)' }}>
          <div className="mb-5 text-center">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-md" style={{ background: 'hsl(15 55% 40%)' }}>
              <Lock className="h-6 w-6 text-white" />
            </div>
            <h2 className="font-display text-lg font-bold text-foreground">管理端登录</h2>
          ...
          <form onSubmit={handlePinSubmit} className="space-y-3.5">
            <div className="relative">
              <input
                ...
                className="w-full rounded border bg-white px-3.5 py-2.5 pr-10 text-sm text-foreground transition focus:outline-none focus:ring-2"
                style={{ borderColor: 'hsl(30 8% 88%)' }}
                placeholder="请输入管理密码"
                autoFocus
              />
            ...
            <button type="submit" className="w-full rounded bg-gray-900 py-2.5 text-sm font-medium text-white transition-colors hover:bg-gray-800">登录</button>
...
function AdminDashboard({ activeTab, setActiveTab }: { activeTab: Tab; setActiveTab: (t: Tab) => void }) {
  ...
  return (
    <div className="min-h-screen" style={{ background: 'hsl(30 20% 98%)' }}>
      <header className="relative overflow-hidden bg-gradient-to-br from-rose-600 via-pink-600 to-rose-500">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-8">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h1 className="font-display text-2xl font-bold tracking-tight text-white">管理端</h1>
          ...
          <div className="flex gap-0.5 overflow-x-auto py-1.5">
            {tabs.map(tab => (
              <button
                ...
                className={`flex flex-shrink-0 items-center gap-1.5 rounded px-3.5 py-2 text-sm font-medium transition-colors ${
                  activeTab === tab.id ? 'bg-primary/12 text-primary' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                }`}
              >
                ...
                {tab.count !== undefined && (
                  ...
                  }`} style={activeTab !== tab.id ? { background: 'hsl(30 10% 92%)' } : undefined}>
                    {tab.count}
                  </span>
                )}
              </button>
...
function DashboardTab({ orders, customers, services: _services, distributors: _distributors, factories }: {
  ...
  const statColors = ['#3b82f6', '#10b981', '#8b5cf6', '#f97316']
    ...
    <div className="animate-fade-in space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 stagger-children">
        {[
          ...
          <div key={s.label} className="rounded-lg bg-white px-5 py-4">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-lg bg-white px-5 py-5">
          <h3 className="font-display mb-4 text-sm font-bold text-foreground">订单状态分布</h3>
          <div className="space-y-2.5">
            {[
              { status: 'pending', label: '待审核', bg: 'bg-amber-100 text-amber-700', bar: 'bg-amber-400' },
              { status: 'approved', label: '已审核', bg: 'bg-blue-100 text-blue-700', bar: 'bg-blue-400' },
              { status: 'processing', label: '生产中', bg: 'bg-violet-100 text-violet-700', bar: 'bg-violet-400' },
              { status: 'completed', label: '已完成', bg: 'bg-emerald-100 text-emerald-700', bar: 'bg-emerald-400' },
              { status: 'delivered', label: '已送达', bg: 'bg-slate-100 text-slate-700', bar: 'bg-slate-400' },
              { status: 'rejected', label: '已拒绝', bg: 'bg-red-100 text-red-700', bar: 'bg-red-400' },
            ].map(s => {
        ...
        <div className="rounded-lg bg-white px-5 py-5">
          <h3 className="font-display mb-4 text-sm font-bold text-foreground">工厂订单分布</h3>
          <div className="space-y-2.5">
            {Object.keys(factoryOrderCounts).length === 0 ? (
              ...
              Object.entries(factoryOrderCounts).map(([name, count], idx) => (
                <div key={name} className="flex items-center gap-3">
                  <span className="flex h-7 w-7 items-center justify-center rounded-md bg-primary/12 text-xs font-bold text-primary">{idx + 1}</span>
      ...
      <div className="rounded-lg bg-white">
        <div className="px-5 py-3.5">
          <h3 className="font-display text-sm font-bold text-foreground">最近订单 ({Math.min(10, orders.length)})</h3>
        </div>
        <div>
          {orders.slice().reverse().slice(0, 10).map(order => (
            <div key={order.id} className="flex items-center justify-between px-5 py-3 transition-colors hover:bg-muted">
              <div>
    ...
    <div className="animate-fade-in">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          <h2 className="font-display text-lg font-bold text-foreground">{roleLabels[role]}账号管理</h2>
          <span className="rounded-full bg-primary/12 px-2.5 py-0.5 text-xs font-semibold text-primary">{accounts.length}</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-48 rounded border bg-white py-2 pl-9 pr-3 text-sm text-foreground transition focus:outline-none focus:ring-2"
              style={{ borderColor: 'hsl(30 8% 88%)' }} placeholder="搜索..."
            />
          </div>
          <button onClick={() => { setEditingId(null); setShowForm(true) }} className="flex items-center gap-1.5 rounded bg-gray-900 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-gray-800">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y" style={{ borderColor: 'hsl(30 8% 90%)' }}>
              {filtered.length === 0 ? (
                ...
                filtered.map(acc => (
                  <tr key={acc.id} className="transition-colors hover:bg-muted">
                    {fields.map(f => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4 backdrop-blur-sm">
          <div className="animate-scale-in w-full max-w-md rounded-lg border bg-white px-6 py-5 shadow-xl" style={{ borderColor: 'hsl(30 8% 90%)' }}>
            <div className="mb-4 flex items-center justify-between">
              <h3 className="font-display text-lg font-bold text-foreground">{editingId ? '编辑' : '添加'}{roleLabels[role]}</h3>
            ...
            <form onSubmit={handleSave} className="space-y-3.5">
                ...
                <div key={f.key}>
                  ...
                  {f.type === 'select' ? (
                    <select name={f.key} required={f.required} defaultValue={defVal}
                      className="w-full rounded border bg-white px-3.5 py-2.5 text-sm text-foreground transition focus:outline-none focus:ring-2" style={{ borderColor: 'hsl(30 8% 88%)' }}>
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => { setShowForm(false); setEditingId(null) }} className="flex-1 rounded border py-2.5 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted" style={{ borderColor: 'hsl(30 8% 88%)' }}>
                  取消
                </button>
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-5 flex items-center justify-between">
        <h2 className="font-display text-lg font-bold text-foreground">全局订单</h2>
        <div className="flex items-center gap-3">
          <button onClick={exportCSV} className="rounded bg-muted px-3.5 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted">
            导出 CSV
          </button>
          <div className="flex gap-0.5 rounded bg-muted p-1">
            {['all', 'pending', 'approved', 'processing', 'completed', 'delivered', 'rejected'].map(s => (
              <button
                key={s}
                onClick={() => setFilter(s)}
                className={`rounded px-3 py-1.5 text-xs font-medium transition-colors ${
                  filter === s ? 'bg-primary/12 text-primary' : 'text-muted-foreground hover:bg-muted'
                }`}
              >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.slice().reverse().map(order => (
            <div key={order.id} className="overflow-hidden rounded-lg bg-white" style={{ borderColor: 'hsl(30 8% 90%)' }}>
              <button
              ...
              {expandedId === order.id && (
                <div className="px-5 py-4" style={{ background: 'hsl(30 12% 97%)' }}>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {order.items.map((item, idx) => (
                      <span key={idx} className="rounded bg-white border px-2 py-0.5 text-xs font-medium text-foreground" style={{ borderColor: 'hsl(30 8% 88%)' }}>
...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { bg: string; text: string; label: string }> = {
    ...
    approved: { bg: 'bg-blue-100', text: 'text-blue-700', label: '已审核' },
    processing: { bg: 'bg-violet-100', text: 'text-violet-700', label: '生产中' },
  ...
  return <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${c.bg} ${c.text}`}>{c.label}</span>
}

function statusLabel(s: string): string {
  const map: Record<string, string> = {
    pending: '待审核', approved: '已审核', processing: '生产中', completed: '已完成', delivered: '已送达', rejected: '已拒绝',
  }
  return map[s] || s
}
