    ...
    return (
      <div className="min-h-screen" style={{ background: 'hsl(30 20% 98%)' }}>
        <div className="mx-auto max-w-2xl px-6 pt-20">
          <div className="rounded-lg bg-white px-8 py-12 text-center">
            <CheckCircle className="mx-auto mb-4 h-16 w-16 text-emerald-500" />
            <h2 className="mb-1.5 font-display text-2xl font-bold text-foreground">订单提交成功！</h2>
            <p className="mb-1" style={{ color: 'hsl(20 6% 48%)' }}>服务机构将尽快审核您的订单</p>
            <p className="mb-6 text-sm" style={{ color: 'hsl(20 6% 52%)' }}>
            ...
            <div className="flex gap-3 justify-center">
              ...
              <button
                ...
                className="rounded-md border bg-white px-5 py-2.5 text-sm font-medium transition-colors hover:bg-muted"
                style={{ borderColor: 'hsl(30 8% 90%)', color: 'hsl(20 6% 48%)' }}
              >
                返回首页
              </button>
            </div>
    ...
    <div className="min-h-screen" style={{ background: 'hsl(30 20% 98%)' }}>
      <header className="border-b bg-white/95 backdrop-blur" style={{ borderColor: 'hsl(30 8% 90%)' }}>
        <div className="mx-auto flex max-w-[1600px] items-center justify-between px-5 py-3.5">
          ...
          <div className="flex items-center gap-3 text-xs" style={{ color: 'hsl(20 6% 48%)' }}>
            {cart.length > 0 && (
              <span className="rounded-full px-2.5 py-0.5 text-xs font-bold text-white" style={{ background: 'hsl(15 55% 40%)' }}>
                {cart.length} 个餐品
              </span>
            )}
          </div>
      ...
      <main className="mx-auto max-w-[1600px] px-4 py-6">
        ...
        <div className="mb-4 flex items-center gap-2">
          <span className="text-sm font-medium" style={{ color: 'hsl(20 6% 48%)' }}>选择周次:</span>
          {[1, 2, 3, 4].map(w => (
            <button
              key={w}
              ...
              style={selectedWeek !== w ? { color: 'hsl(20 6% 48%)' } : undefined}
            >
              Week {w}
            </button>
          ))}
        ...
        <div className="mb-5 rounded-lg bg-white px-5 py-4">
          <h2 className="font-display text-xl font-bold text-foreground mb-1.5">CCA冬季菜单 - Week {selectedWeek}</h2>
          <p className="text-sm mb-3" style={{ color: 'hsl(20 6% 50%)' }}>Hot-Chilled Winter Menu 2026 CHSP</p>
          <div className="grid gap-2 text-xs md:grid-cols-2">
            ...
            <div className="flex items-center gap-1.5">
              <span className="rounded px-1.5 py-0.5 font-mono" style={{ background: 'hsl(15 55% 40% / 0.08)', color: 'hsl(15 55% 40%)' }}>(GF)</span>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <div>
            ...
            />
            <div className="mt-3 rounded border border-blue-200 bg-blue-50 px-3.5 py-2.5 text-sm text-blue-800">
          ...
          <div className="lg:sticky lg:top-6 lg:self-start">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-lg bg-white/70 py-14 text-center">
                <Package className="mb-2.5 h-9 w-9" style={{ color: 'hsl(30 8% 80%)' }} />
                <p className="text-sm font-medium" style={{ color: 'hsl(20 6% 50%)' }}>尚未选择餐品</p>
                <p className="mt-1 text-xs" style={{ color: 'hsl(20 6% 56%)' }}>点击左侧菜单中的餐品开始点餐</p>
              </div>
            ) : (
              <div className="rounded-lg bg-white">
                ...
                <div className="px-4 py-3.5">
                  <h3 className="flex items-center gap-2 font-display text-sm font-semibold text-foreground">
                    <Package className="h-4.5 w-4.5" style={{ color: 'hsl(15 55% 40%)' }} />
                    已选餐品
                    <span className="rounded-full px-2 py-0.5 text-xs font-bold text-white" style={{ background: 'hsl(15 55% 40%)' }}>
                ...
                <div className="max-h-[320px] space-y-3 overflow-y-auto p-4">
                  {cartByDay().map(group => (
                    <div key={group.day}>
                      <p className="mb-1.5 text-xs font-semibold" style={{ color: 'hsl(15 55% 40%)' }}>{group.day}</p>
                      <div className="space-y-1.5">
                        {group.items.map(item => {
                          const meal = mockMeals.find(m => m.id === item.mealId)!
                          return (
                            <div key={item.mealId} className="flex items-center justify-between rounded border px-3 py-2.5" style={{ borderColor: 'hsl(30 8% 91%)', background: 'hsl(30 12% 97%)' }}>
                              <div className="min-w-0 flex-1 pr-2">
                                <p className="truncate text-sm font-medium text-foreground">{meal.name}</p>
                                <p className="text-xs" style={{ color: 'hsl(20 6% 50%)' }}>${meal.price} x {item.quantity}</p>
                              </div>
                              <div className="flex items-center gap-1">
                                <button
                                  type="button"
                                  onClick={() => removeFromCart(item.mealId)}
                                  className="flex h-5.5 w-5.5 items-center justify-center rounded border bg-white text-xs transition-colors hover:bg-muted"
                                  style={{ borderColor: 'hsl(30 8% 88%)', color: 'hsl(20 6% 48%)' }}
                                >
                                  -
                                </button>
                                ...
                                <button
                                  ...
                                  className="flex h-5.5 w-5.5 items-center justify-center rounded border text-xs transition-colors"
                                  style={{ borderColor: 'hsl(15 55% 40% / 0.3)', background: 'hsl(15 55% 40% / 0.06)', color: 'hsl(15 55% 40%)' }}
                                >
                                  +
                                </button>
                                <button
                                  ...
                                  className="ml-0.5 flex h-5.5 w-5.5 items-center justify-center rounded text-xs transition-colors hover:bg-red-50 hover:text-red-500"
                                  style={{ color: 'hsl(30 8% 70%)' }}
                                  title="删除"
                                >
                                  <X className="h-3 w-3" />
                                </button>
                ...
                <div className="px-4 py-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm" style={{ color: 'hsl(20 6% 50%)' }}>合计</span>
                    <span className="text-lg font-bold" style={{ color: 'hsl(15 55% 40%)' }}>${getCartTotal()}</span>
                  </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="px-4 py-4">
                  ...
                  <div className="space-y-2.5">
                    <input
                      ...
                      className="w-full rounded border px-3 py-2 text-sm text-foreground transition focus:outline-none focus:ring-2"
                    ...
                    <textarea
                      ...
                      className="w-full resize-none rounded border px-3 py-2 text-sm text-foreground transition focus:outline-none focus:ring-2"
                      style={{ borderColor: 'hsl(30 8% 88%)' }}
                      rows={3}
                      placeholder="配送地址 *"
                    />
                  ...
                  <button
                    type="submit"
                    className="mt-3.5 w-full rounded-md bg-gray-900 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-gray-800"
                  >
                    提交订单
                  </button>
                </form>
